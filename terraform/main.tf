...
resource "null_resource" "provision_docker" {
 ...
 provisioner "remote-exec" {
  inline = [
    ...
    "git clone https://github.com/zamibd/server.git /root/server",
    "cd /root/server/app",
    "rm -f index.php",
    "git clone https://github.com/zamibd/cloud-sms.git /root/server/app/cloud-sms", # change your app repo and 
    "cp -a /root/server/app/cloud-sms/. /root/server/app/",                         #change file name
    "rm -rf /root/server/app/cloud-sms",                                            #change file name

    
    # Start Docker containers
    "cd /root/server && docker compose up -d --build"                               # change file name 
  ]
}
}
